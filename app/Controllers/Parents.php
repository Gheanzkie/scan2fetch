...
class Parents extends BaseController
{
    ...
    public function save()
    {
        ...
        $parentId = $this->parentsModel->insert([
            ...
            'picture' => $pictureName, 'qr_code' => $qrValue, 'created_by' => session('user_id'),
        ]);
        ...
        if ($fnames) {
            ...
            foreach ($fnames as $i => $fname) {
                $this->studentModel->save([
                    'fname' => $fname, 'mname' => $mnames[$i] ?? null, 'lname' => $lnames[$i],
                    'grade_section' => $grades[$i], 'parent_id' => $parentId, 'created_by' => session('user_id'),
                ]);
            }
        }
        $this->logModel->addLog(session('user_id'), session('fname').' '.session('lname'), session('role'), 'register', 'parent', 'Registered parent: '.$this->request->getPost('fname').' '.$this->request->getPost('lname'));
    ...
    public function update()
    {
        ...
        $fnames = $this->request->getPost('student_fname');
        if ($fnames) {
            $mnames = $this->request->getPost('student_mname'); $lnames = $this->request->getPost('student_lname'); $grades = $this->request->getPost('grade_section');
            foreach ($fnames as $i => $fname) {
                if (!empty($studentIds[$i])) {
                    $this->studentModel->update($studentIds[$i], ['fname' => $fname, 'mname' => $mnames[$i] ?? null, 'lname' => $lnames[$i], 'grade_section' => $grades[$i]]);
                }
            }
        }
    ...
    {
        $id = $this->request->getPost('id'); $pictureName = null;
        ...
        if ($pictureName) $this->parentsModel->update($id, ['picture' => $pictureName]);
    ...
    public function logs()
    {
        ...
        $data['releases'] = $this->fetchLogModel->select('fetch_logs.*, students.fname as sfname, students.lname as slname')
            ->join('students', 'students.id = fetch_logs.student_id')
            ->where('students.parent_id', $userId)->orderBy('fetch_logs.time_released', 'DESC')->limit(50)->findAll();
