                        ...
                        <div class="card-body pt-2">
                            ...
                            <div class="row mb-3">
                                <div class="col-md-3 mb-2">
                                    <input type="text" id="searchFilter" class="form-control form-control-sm" placeholder="Search student, parent, fetcher...">
                                </div>
                            ...
                            <div class="table-responsive">
                                <table class="table table-hover table-sm mb-0" id="authTable">
                                        ...
                                        <tr class="auth-row" 
                                            ...
                                            <td>
                                                <?php if (!empty($a['fetcher_picture'])): ?>
                                                    <img src="<?= base_url('uploads/fetchers/'.$a['fetcher_picture']) ?>" 
                                                         class="img-circle mr-1" 
                                                         style="width:40px;height:40px;object-fit:cover;cursor:pointer;border:2px solid #667eea;" 
                                                         onclick="openImageViewer('<?= base_url('uploads/fetchers/'.$a['fetcher_picture']) ?>', '<?= esc($a['fetcher_fname'].' '.$a['fetcher_lname']) ?>')"
                                                         title="Click to view full photo">
                                                <?php else: ?>
                                                    <div class="img-circle bg-light d-inline-flex align-items-center justify-content-center mr-1" style="width:40px;height:40px;border:2px solid #667eea;">
                                            ...
                                            <td>
                                                <span class="badge badge-<?= $status == 'approved' ? 'success' : ($status == 'released' ? 'info' : ($status == 'declined' ? 'danger' : 'warning')) ?>">
                                                    <?= ucfirst($status ?: '—') ?>
                                                </span>
                                            </td>
                                            ...
                                            <td class="text-center">
                                                <?php if ($status == 'pending'): ?>
                                                    <button class="btn btn-outline-secondary btn-xs action-btn" data-id="<?= $a['id'] ?>" data-action="pending" data-name="<?= esc($a['fetcher_fname'].' '.$a['fetcher_lname']) ?>">
                                                        <i class="fas fa-ellipsis-h"></i> Action
                                                    </button>
                                                <?php elseif ($status == 'approved'): ?>
                                                    <button class="btn btn-outline-secondary btn-xs action-btn" data-id="<?= $a['id'] ?>" data-action="approved" data-name="<?= esc($a['fetcher_fname'].' '.$a['fetcher_lname']) ?>">
                                                        <i class="fas fa-ellipsis-h"></i> Action
                                                    </button>
                                                <?php else: ?>
                                                    <span class="badge badge-<?= $status == 'released' ? 'success' : 'danger' ?>">
                                                        <?= ucfirst($status) ?>
                                                    </span>
                                                <?php endif; ?>
                                            </td>
    ...
    <div class="modal-dialog modal-dialog-centered modal-sm">
        <div class="modal-content border-0 shadow">
            ...
            <div class="modal-body text-center">
                ...
                <p class="text-muted small mb-0"><strong id="confirmAuthName"></strong></p>
            </div>
    ...
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content border-0 shadow" style="background:#1a1a2e;">
            <div class="modal-header border-0" style="background:#1a1a2e;">
                <h5 class="text-white"><i class="fas fa-image mr-2"></i><span id="imageViewerTitle">Fetcher Photo</span></h5>
            ...
            <div class="modal-footer border-0" style="background:#1a1a2e;">
                <a id="imageDownloadBtn" href="" download="fetcher.png" class="btn btn-primary btn-sm"><i class="fas fa-download"></i> Download</a>
...
$(function() {
    ...
    $(document).on('click', '.action-btn', function() {
        pendingAuthId = $(this).data('id');
        pendingAction = $(this).data('action');
        $('#confirmAuthName').text($(this).data('name'));
        ...
        if (pendingAction === 'pending') {
            buttons = `
                ...
                <button type="button" class="btn btn-success btn-sm" id="confirmApprove"><i class="fas fa-check mr-1"></i> Approve</button>`;
        } else if (pendingAction === 'approved') {
            buttons = `
                <button type="button" class="btn btn-danger btn-sm" id="confirmDecline"><i class="fas fa-times mr-1"></i> Decline</button>
                <button type="button" class="btn btn-info btn-sm" id="confirmRelease"><i class="fas fa-door-open mr-1"></i> Release</button>`;
        }
    ...
    $(document).on('click', '#confirmApprove', function() {
        ...
        window.location.href = '<?= base_url('authorization-approve/') ?>' + pendingAuthId;
    });

    $(document).on('click', '#confirmRelease', function() {
        if (!pendingAuthId) return;
        if (confirm('Release? SMS will be sent.')) {
            window.location.href = '<?= base_url('authorization-release/') ?>' + pendingAuthId;
        }
    });

    $(document).on('click', '#confirmDecline', function() {
        if (!pendingAuthId) return;
        if (confirm('Decline? SMS will be sent.')) {
            window.location.href = '<?= base_url('authorization-decline/') ?>' + pendingAuthId;
        }
    });
});
...
function openImageViewer(imageUrl, title) {
    ...
    $('#imageViewerTitle').text(title || 'Fetcher Photo');
